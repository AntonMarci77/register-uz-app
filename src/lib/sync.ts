 ...
 */
// ============================================================
// Backfill reportov — doplnenie obsahu (titulná strana + tabuľky)
// ============================================================

/**
 * Backfill pre reporty, ktoré majú NULL obsah.
 * Stiahne ich znova z API a aktualizuje tsNazovUJ, tsICO, tabulky atď.
 *
 * @param batchSize - Koľko reportov spracovať v jednom behu (default 500)
 * @param concurrency - Počet súbežných API requestov (default 30)
 * @param onProgress - Callback pre progress reporting
 * @returns Počet úspešne aktualizovaných reportov
 */
export async function backfillReportContent(
  batchSize: number = 500,
  concurrency: number = 30,
  onProgress?: (processed: number, updated: number, total: number) => void
): Promise<{ processed: number; updated: number; failed: number; remaining: number }> {
  console.log(`[BACKFILL] Starting report content backfill (batch=${batchSize}, concurrency=${concurrency})`);

  // 1. Get IDs of reports missing content (tsNazovUJ IS NULL as proxy)
  const missingRows = await prisma.$queryRawUnsafe<{ id: number }[]>(
    `SELECT id FROM "FinancialReport"
     WHERE zmazany = false
       AND "tsNazovUJ" IS NULL
     ORDER BY id ASC
     LIMIT $1`,
    batchSize
  );
  ...
  if (missingRows.length === 0) {
    console.log("[BACKFILL] No reports need backfilling");
    // Count remaining just to be sure
    const remainingResult = await prisma.$queryRawUnsafe<{ count: string }[]>(
      `SELECT COUNT(*) as count FROM "FinancialReport" WHERE zmazany = false AND "tsNazovUJ" IS NULL`
    );
    return { processed: 0, updated: 0, failed: 0, remaining: parseInt(remainingResult[0]?.count || "0") };
  }

  const ids = missingRows.map(r => r.id);
  console.log(`[BACKFILL] Found ${ids.length} reports to backfill (IDs ${ids[0]}..${ids[ids.length - 1]})`);
  ...
  for (let i = 0; i < ids.length; i += concurrency) {
    ...
    const results = await Promise.allSettled(
      chunk.map(id => getFinancialReport(id))
    );
    ...
    for (const result of results) {
      ...
      // Only count as updated if we actually got content
      const hasContent = tS?.nazovUctovnejJednotky || tS?.ico || (tabulky && tabulky.length > 0);

      updateSqls.push(`
        UPDATE "FinancialReport" SET
          "tsNazovUJ" = ${sqlStr(tS?.nazovUctovnejJednotky, 500)},
          "tsICO" = ${sqlStr(tS?.ico, 8)},
          "tsPravnaForma" = ${sqlStr(tS?.pravnaForma, 100)},
          "tsSkNace" = ${sqlStr(tS?.skNace, 100)},
          "tsTypZavierky" = ${sqlStr(tS?.typZavierky, 30)},
          "tsKonsolidovana" = ${sqlBool(tS?.konsolidovana)},
          "tsTypUctovnejJednotky" = ${sqlStr(tS?.typUctovnejJednotky, 30)},
          "tsObdobieOd" = ${sqlStr(tS?.obdobieOd, 7)},
          "tsObdobieDo" = ${sqlStr(tS?.obdobieDo, 7)},
          "tsPredchObdobieOd" = ${sqlStr(tS?.predchadzajuceObdobieOd, 7)},
          "tsPredchObdobieDo" = ${sqlStr(tS?.predchadzajuceObdobieDo, 7)},
          "tabulky" = ${sqlJson(tabulky)}
    ...
    if (updateSqls.length > 0) {
      try {
        await prisma.$transaction(
          updateSqls.map(sql => prisma.$executeRawUnsafe(sql))
        );
      } catch (err) {
        console.error("[BACKFILL] Transaction error, falling back to individual:", err);
        for (const sql of updateSqls) {
          try {
            await prisma.$executeRawUnsafe(sql);
          } catch (singleErr) {
            console.error("[BACKFILL] Single update error:", singleErr);
          }
        }
      }
    }
    ...
    if (onProgress) {
      onProgress(processed, updated, ids.length);
    }

    if (processed % 100 === 0 || processed === ids.length) {
      console.log(`[BACKFILL] Progress: ${processed}/${ids.length} processed, ${updated} with content, ${failed} failed`);
    }
  }
  ...
  console.log(`[BACKFILL] Done: ${processed} processed, ${updated} with content, ${failed} failed, ${remaining} remaining`);

  return { processed, updated, failed, remaining };
}
